...
for row in input {
    rowLoop: for number in row {
        for comparedNumber in row {
            if number == comparedNumber { continue }
            
            if number % comparedNumber == 0 {
                checksum += number / comparedNumber
                break rowLoop
            }
        }
    }
